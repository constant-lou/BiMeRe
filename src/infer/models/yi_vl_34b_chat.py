...
def load_model(model_name, model_args, use_accel=True):
    ...
    if num_gpus <= 1 and use_accel:
        print("Warning: Only one or zero GPUs found. Attempting to load, but 34B likely requires multiple GPUs.")
    ...
    device_map_config = "auto" if use_accel and num_gpus > 1 else ("cuda:0" if use_accel and num_gpus == 1 else "cpu")

    print(f"[INFO] Using device_map: '{device_map_config}', dtype: {dtype}")
    ...
    try:
        ...
        print("[INFO] Model loading with device_map initiated...")

    except TypeError as te:
         print(f"[WARNING] `load_pretrained_model` might not accept `device_map` or `torch_dtype`. Error: {te}")
         print("[INFO] Trying `load_pretrained_model` without extra args and relying on its internal logic (might fail or load to CPU).")
         tokenizer, model, image_processor, context_len = load_pretrained_model(model_path)
         print("[WARNING] Model loaded without explicit device_map. If it's on CPU, it won't fit a single GPU.")
    ...
    try:
        ...
        print(f"[INFO] Model loaded. First parameter is on: {first_param_device}.")
        if "cuda" in str(first_param_device):
            print("[INFO] Model appears to be loaded across GPUs (or at least started on one).")
        else:
            print("[WARNING] Model might be loaded on CPU. Inference will be slow or OOM if moved.")
    except Exception:
        print("[INFO] Model loaded (unable to determine exact device distribution).")
...
def infer(prompts, **kwargs):
    ...
    for prompt_item in prompts:
        ...
        if DEFAULT_IMAGE_TOKEN in conv.messages[-2][1] and not images_paths:
            raise ValueError("Image token present but no image paths found.")
        elif not images_paths and DEFAULT_IMAGE_TOKEN not in conv.messages[-2][1]:
...
if __name__ == '__main__':
    print("Yi-VL 34B script with 6B's loading & prompt logic.")
    pass